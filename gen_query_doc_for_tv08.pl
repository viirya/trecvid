...
GetOptions ("y=s" => \$tv_year, "rs=i" => \$rs);
...
foreach my $topic_id (keys %queries) {
...
sub get_related_words_from_wikipedia {
  ...
  my $wiki_result = $wiki->search($query);
  if (!defined $wiki_result) {
    $wiki_result = $wiki->search($query . '_(disambiguation)');
    return \[] if !defined $wiki_result;
  }
  ...
  my $min_index = scalar(@sorted_keys) - 1; #/ 2;

  my @items = @sorted_keys[$min_index..$max_index];
  print Dumper(@items);
  foreach (@items) {
    my $recursive_ret = get_related_words_from_wikipedia($_, $times - 1);
    push @ret, @{$recursive_ret};
  }
  print Dumper(@ret);
