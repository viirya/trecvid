...
#last if $count++ > 0;
     ...
     if ($topic_id eq '0250') {
       ...
       foreach my $keyword (@dup_removed_stopwords) {
         my $syn_words = get_synset_of_wordnet($keyword, '../WordNet/dict');
         my $definitions = get_definition_of_wordnet($keyword, '../WordNet/dict');

         my $sentence = $engine->sentence(join ' ', @{$definitions});
         my $tokens = $sentence->word_tokens();
         foreach my $token (keys %{$tokens}) {
           if ($tokens->{$token} eq 'NN') {
             print $token . "\n";
             $query .= ' ' . $token;
           }
         }
...
#if ($index_dir eq 'index_tv08_s15') {
#  print $_ . "\n";
#  print "shot: $subshot_id score: $score \n";
#}
...
#print Dumper(%subshot_rank_iteration) if ($index_dir eq 'index_tv08_s15');
...
foreach $topic_id (keys %queries) {
  ...
  my @subshot_sorted_ids = sort {
    #print "$topic_id $a: " .  $subshot_rank{$topic_id}{$a} . " $b: " . $subshot_rank{$topic_id}{$b}. "\n";
     
    $subshot_rank{$topic_id}{$a} <=> $subshot_rank{$topic_id}{$b};
 
  } keys %{$subshot_rank{$topic_id}};
...
sub get_definition_of_wordnet {
  ...
  foreach $type (@querystring_types) {
    ...
    foreach $meaning (@meanings) {
      my @syn = $wn->querySense($meaning, "glos");
      foreach $word (@syn) {

        my @removed_stopwords = grep { !$stopwords->{$_} } split /\s/, $word;
        foreach (0..scalar(@removed_stopwords)-1) {
          $removed_stopwords[$_] =~ s/(.*)\,(.*)/$1$2/;
          $removed_stopwords[$_] =~ s/(.*)\-(.*)/$1$2/;
          $removed_stopwords[$_] =~ s/(.*)\.(.*)/$1$2/;
          ...
          $removed_stopwords[$_] =~ s/(.*)\)(.*)/$1$2/;
          $removed_stopwords[$_] =~ s/(.*)\/(.*)/$1$2/;
          $removed_stopwords[$_] =~ s/(.*)\"(.*)/$1$2/g;
        }

        push @ret, @removed_stopwords;
      }
    }
  }
