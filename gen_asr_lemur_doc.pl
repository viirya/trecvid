...
goto test;
    ...
    foreach (0..($subshot_end - $subshot_start)) {
      if (defined $asr_by_subshot_id{$subshot_index + $_}) {
        my @asr_of_subshot = split ' ', $asr_by_subshot_id{$subshot_index + $_};
    ...
    foreach (0..scalar(@{$asr})-1) {
      if (!defined $asr_hash{trim($asr->[$_])}) {
        delete $asr->[$_];
      }
    }
...
=cut
    ...
    foreach (0..($subshot_end - $subshot_start)) {
      ...
      my @removed_stop_keywords_asr_mt = @asr_of_subshot; #grep { !$stopwords->{$_} } @asr_of_subshot;
      $asr_by_subshot_id{$subshot_index + $_} .= ' ' . join ' ', @removed_stop_keywords_asr_mt; # . ' ' . $asr_by_subshot_id{$subshot_index + $_}; 
...
#print "subshot: $subshot_id asr: " . $asr_by_subshot_id{$subshot_id} . "\n";
...
END
    ...
    if (length($doc) >= 10000000) {
      $doc >> io($output_filename); #("lemur_test/$tv_year.txt");
      $doc = '';
    } 
  }
