...
foreach $topic_id (keys %queries) {
   ...
   @query_words = grep { !$stopwords->{$_} } @query_words;

   my $expand_query;
   foreach $query_word (@query_words) {
     print "expanding $query_word\n";
     my $ret = get_synset_of_wordnet($query_word, '../WordNet/dict');
     $expand_query .= join ' ', @{$ret};
   }
   print "after expansion: $expand_query\n";
   $query = $expand_query;
